...
| CLI-024 | `drydock analyze <Target>` runs Scrum-team Blueprint analysis; emits quality signal (Blocked/Questions/Ready), story list, ANALYSIS.md, SEA_TRIALS.md, SOUNDINGS.md, COMPASS.md (if absent/unpopulated), four fixed spike-*.json questionnaires, optional variable spikes; injects BUILD_CONFIGURATION.md as prior PO answers; advances METADATA.md lifecycle state to "analyzed" (forward-only) and fills captains_chair.html template when state advances; read-only w.r.t. blueprint/ and MANIFEST.md | IMPLEMENTED | `test_analyze.py` — unit, CLI contract, file-write, lifecycle, compass-detection, captain's chair; `test_metadata.py` lifecycle state; `test_manifest.py` node graph model |
