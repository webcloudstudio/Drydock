...
| CLI-025 | `drydock import <Blueprint> <Target> <Source> --format <auto\|markdown\|source\|speckit>` preserves Markdown source bundles under `<Target>/blueprint/sources/`; `--format source` assembles LLM prompt from source code and writes Blueprint files; `--format speckit` translates Spec Kit project to Blueprint with conversion report; `--format auto` detects format from source layout | DONE | `test_import_markdown.py`, `test_import_source.py`, `test_import_speckit.py`, `test_cli.py::TestImport`, `test_cli.py::TestPlanningSession::test_markdown_import_plan_create_and_approve` |
