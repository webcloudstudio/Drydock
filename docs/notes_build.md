...
Prototyper's `BUILD_PLAN_INTENT`. It is **generated by `plan create`** (the rich build-step
narrative — numbered phases with `stories`/`stack`/`rules`) at the Target root and edited in
QuarterDeck story-planning; it is distinct from `PLAN_COMPASS.md` (human steering prose for
`plan create`). It separates two kinds of content:
