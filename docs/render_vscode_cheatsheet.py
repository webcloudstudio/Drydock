...
def card(ax, x, top, width, height, title, rows, *, accent=TEAL, note=None):
    ...
    ax.add_patch(
        FancyBboxPatch(
            ...
            linewidth=0.8, edgecolor=BORDER, facecolor="#FCFEFF",
        )
    )
    header_h = 0.038
    ax.add_patch(
        FancyBboxPatch(
            (x, top - header_h), width, header_h,
            boxstyle="round,pad=0.004,rounding_size=0.012",
            linewidth=0, facecolor=PALE_BLUE,
        )
    )
    ax.text(x + 0.012, top - header_h / 2, title, va="center", ha="left",
            fontsize=9.3, fontweight="bold", color="#183B56")

    y = top - header_h - 0.011
    row_h = 0.027
    for label, key in rows:
        ax.text(x + 0.012, y - row_h / 2, label, va="center", ha="left",
                fontsize=7.55, color=INK)
        key_width = min(width * 0.54, 0.0082 * len(key) + 0.028)
        key_x = x + width - key_width - 0.010
        ax.add_patch(
            FancyBboxPatch(
                (key_x, y - row_h * 0.79), key_width, row_h * 0.61,
                boxstyle="round,pad=0.002,rounding_size=0.004",
                linewidth=0.6, edgecolor="#AEBFCB", facecolor="#FFFFFF",
            )
        )
        ax.text(key_x + key_width / 2, y - row_h / 2, key, va="center", ha="center",
                fontsize=6.85, family="DejaVu Sans Mono", fontweight="bold", color="#12304A")
        y -= row_h
    if note:
        note_h = max(0.050, height - (top - y) - 0.012)
        ax.add_patch(
            FancyBboxPatch(
                (x + 0.009, bottom + 0.009), width - 0.018, note_h,
                boxstyle="round,pad=0.004,rounding_size=0.006",
                linewidth=0, facecolor=note[0],
            )
        )
        ax.text(x + 0.017, bottom + 0.009 + note_h / 2, note[1], va="center", ha="left",
                fontsize=7.0, color=note[2], wrap=True)
...
def main():
    ...
    ax.add_patch(FancyBboxPatch((0.03, 0.885), 0.94, 0.083,
    ...
    ax.text(0.918, 0.926, "WINDOWS + WSL", fontsize=7.6, color="#E8FFFF", va="center", ha="center",
            fontweight="bold", bbox={"boxstyle": "round,pad=0.42", "fc": "none", "ec": "#A7E4E4", "lw": 0.8})

    x1, x2, x3, width = 0.03, 0.353, 0.676, 0.294
    card(ax, x1, 0.862, width, 0.294, "Navigate the codebase", [
    ...
    card(ax, x1, 0.555, width, 0.205, "Window and views", [
    ...
    card(ax, x1, 0.340, width, 0.230, "Explorer and Outline", [
    ...
    ], note=("#ECF9F6", "Use Outline for headings and symbols. Use breadcrumbs for path + symbol context.", "#15594F"))

    card(ax, x2, 0.862, width, 0.267, "Test and terminal: your bindings", [
    ...
    card(ax, x2, 0.585, width, 0.205, "Editing, copy, and Vim", [
    ...
    ax.add_patch(FancyBboxPatch((x2, 0.265), width, 0.103,
                                boxstyle="round,pad=0.004,rounding_size=0.012",
                                linewidth=0.7, edgecolor="#F0D29D", facecolor="#FFF8E8"))
    ax.text(x2 + 0.014, 0.334, "TERMINAL RULE", fontsize=8.3, color=GOLD, fontweight="bold", va="center")
    ax.text(x2 + 0.014, 0.304, "You are WSL-first. Use the integrated Ubuntu terminal\nfor project commands; it starts at the workspace folder.",
            fontsize=7.5, color="#68450E", va="center")

    card(ax, x3, 0.862, width, 0.172, "Codex composer", [
        ("Send a request", "Enter or Ctrl+Enter"),
        ("Insert a new line", "Shift+Enter"),
    ], note=("#FFF8E8", "Before changing Codex WSL settings: save editor work. VS Code reloads and recent chat UI state can disappear.", "#68450E"))
    card(ax, x3, 0.680, width, 0.218, "Three habits worth keeping", [
