...
`--flatten` renders the output as a deterministic multi-page HTML site. The requested output path is the landing page. Drydock writes one page for each Markdown H1 or H2 section under an output-specific section subdirectory. Content before the first H1 or H2 becomes an `Introduction` section. A document with no H1 or H2 sections becomes a single `Introduction` section. The landing page and section pages include a logo/table-of-contents navigation column whose links open the generated section pages.
