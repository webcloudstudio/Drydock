...
| `drydock plan approve\|revise\|reject <Blueprint> <Target> [<Feedback>]` | Decide the complete Planning Session plan |
...
`BUILD_PLAN.md`. It also creates the target workspace and generates its QuarterDeck Planning
Session. A draft plan has no runnable frontier. Whole-plan approval establishes the executable
baseline and makes its frontier runnable.
...
All four use the same four states:
...
`scope:` declares whether a story changes the Blueprint, target software, or both.
...
3. `drydock plan init <Blueprint>` inventories conformed specifications and preserved source inputs.
4. `drydock plan create <Blueprint> <Target>` creates the draft executable plan and Planning Session.
5. Review and approve the complete plan, then proceed through its runnable frontier.
...
2. `drydock plan init <Blueprint>` inventories and orders the available planning inputs.
3. `drydock plan create <Blueprint> <Target>` creates the smallest safe draft decomposition.
   Trivial work may be one story plus AC; larger work may use optional feature parents and spikes.
4. Review the draft in the target-local QuarterDeck Planning Session. Revise or reject as needed,
   then approve the complete plan.
5. `drydock build <Blueprint> <Target>` executes the approved frontier. Stories that create or
...
`drydock plan create` first creates a draft executable plan and a target-local Planning Session.
The QuarterDeck presents optional features, executable stories and spikes, dependencies, and nested
acceptance gates. The product owner approves the whole plan before any frontier is runnable.
`drydock build` then runs the approved frontier and stops at review gates.
...
2. The product owner approves, revises, or rejects the complete decomposition. Approval exposes the
   runnable frontier.
...
4. The QuarterDeck surfaces each completed object with the evidence and review material needed for the
   next decision. The product owner approves, revises, or rejects; decisions write back to
   `BUILD_PLAN.md`.
5. Repeat until all objects and optional feature parents are accepted.
...
the draft decomposition and applies whole-plan approve, revise, or reject decisions through the
same authoritative plan-state writer. Ordinary QuarterDeck review controls do not approve a plan.
