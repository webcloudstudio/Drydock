...
Decisions made during quarterdeck thinkthrough that materially extend or supersede sections above.

### A1. Expanded analyze output contract

The prior design (section 4) has analyze writing only `ANALYSIS.md` + `planning.json`. The new
contract adds five more outputs:

| Output | Location | Notes |
|---|---|---|
| COMPASS.md | `COMPASS.md` (target root) | Generated if absent; supersedes §3.3 which put COMPASS.md in blueprint/ |
| ANALYSIS.md | `QuarterDeck/planning/ANALYSIS.md` | Unchanged |
| SEA_TRIALS.md | `SEA_TRIALS.md` | NEW — LLM-generated strategic objectives; not human-authored |
| SOUNDINGS.md | `SOUNDINGS.md` | NEW — AC blocks projected from spec; previously attributed to plan create |
| spike-intent.json | `QuarterDeck/questionnaires/spike-intent.json` | NEW — replaces generic planning.json |
| spike-stack.json | `QuarterDeck/questionnaires/spike-stack.json` | NEW |
| spike-gaps-ac.json | `QuarterDeck/questionnaires/spike-gaps-ac.json` | NEW |
| spike-guardrails.json | `QuarterDeck/questionnaires/spike-guardrails.json` | NEW |
| spike-<slug>.json | `QuarterDeck/questionnaires/spike-<slug>.json` | NEW — variable open questions |

Does not touch: `BUILD_PLAN.md`, `tickets.json`, `console.yaml`.

### A2. Spikes replace the generic questionnaire

Each questionnaire file IS a spike story in the plan structure. Each has a specific LLM analytical
goal (not a generic question dump). The four fixed spikes always run; additional spikes are
configurable via Rigging or analyze config. Variable spikes auto-surface in QuarterDeck via a
`sources:` glob on `QuarterDeck/questionnaires/`.

### A3. COMPASS.md role and location

COMPASS.md is the project constitution/system prompt — not an intent file. It lives at the Target
root (IMPLEMENTED). It is injected into every LLM run across all commands as ambient project
context. Analyze generates it from the spec if absent (warn user; continue). The prior location
`blueprint/COMPASS.md` is superseded.

### A4. Single LLM call, multiple output blocks

All analyze outputs produced in one LLM call using delimited output blocks (extending the existing
`=== ANALYSIS.md ===` pattern to cover all files). The module parses and writes each file
deterministically. Chosen for token efficiency over per-spike calls.

### A5. Soundings ownership moves to analyze

Soundings (AC blocks) are populated by `analyze`, not `plan create`. This supersedes the table in
section 2 which attributes SOUNDINGS to plan create. `plan create` reads the analyze outputs and
assembles BUILD_PLAN.md.

### A6. init TODO

`drydock init` currently pre-creates `blueprint/sources/.gitkeep` and related blueprint
subdirectory artifacts. When analyze is rebuilt, init should not pre-create blueprint structure
that analyze owns. Defer until analyze implementation.
