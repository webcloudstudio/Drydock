...
- *Emits:* `BUILD_PLAN_COMPASS.md` — the single build-step report that prepares `drydock build`.

`BUILD_PLAN_COMPASS.md` defines the efficient, human-reviewable build path. It is not a flat
inventory. It contains numbered build steps in execution order. Each step is a coherent, testable
group of one or more Manifest stories that can be built and verified together without exceeding the
configured prompt budget.

The first step establishes initial conditions before product capability work: package or runtime
scaffold, test harness, architecture boundary, configuration, persistence foundation, and a
minimal smoke check. For a web application, it creates the application factory and health check
before feature routes or screens. Build shared providers before their consumers; build screens only
after their backing providers; defer secondary, reporting, documentation, compatibility, and help
features until the core user path works.

Use this exact report structure. Preserve the field names and comment format:

```text
# BUILD_PLAN_COMPASS.md — <Target>
#
# GENERATED BY: drydock plan create
# DO NOT EDIT THIS FILE DIRECTLY.
# To change build order or grouping: edit MANIFEST_COMPASS.md, then re-run drydock plan create <Target>.
#
## 1: Foundation
stories: scaffold, persistence
stack: common.md (7k), python.md (10k)
rules: BUSINESS_RULES.md, <applicable build rule files>
specifications: ARCHITECTURE.md (8k), DATABASE.md (10k)
context: COMPASS.md (2k)
instructions: |
  Establish only the executable foundation and its focused tests. Do not implement downstream
  features, screens, reporting, documentation, or compatibility adapters in this step.
smoke:
  - python -m pytest tests/test_foundation.py -q
# Estimated prompt: ~42KB (~11k tokens)

## 2: <Coherent capability group>
stories: <Manifest story ids in this step>
stack: <applicable stack files with rounded kB sizes>
rules: <applicable Rigging and prompt rule files>
specifications: <implemented Blueprint files with rounded kB sizes>
context: <read-only supporting files with rounded kB sizes, or none>
instructions: |
  <Bounded implementation objective and explicit exclusions.>
smoke:
  - <executable command or test>
# Estimated prompt: ~<kB>KB (~<tokens>k tokens)
```

Rules for the Compass:

- Every `stories:` entry is a real `MANIFEST.md` story id. Each story appears in exactly one step.
- Steps are topologically ordered by Manifest `depends:`. A later step never supplies a dependency
  to an earlier step.
- Group dependent stories only when they share stack, specifications, and bounded context. Keep
  unrelated capabilities in separate steps even when they are independently runnable.
- Include actual authored file sizes rounded to the nearest whole kB in `stack:`,
  `specifications:`, and `context:`. Estimate the prompt as their sum plus the listed rules and
  instructions; show both kB and tokens using approximately four bytes per token.
- Keep every estimated prompt at or below `PROMPT_WARN_KB`. If the initial plan exceeds that budget,
  split the step before emitting the Compass.
- `instructions:` names the implementation boundary for the group and what must not be built yet.
- `smoke:` contains at least one executable verification command for every step.
- The final steps contain non-core work such as documentation, reporting, compatibility adapters,
  and help surfaces unless an explicit dependency requires them earlier.
- Do not include an `AGENTS.md`-only step. It is generated with the capability surface and belongs
  in the owning implementation step.
