...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--program", dest="program", nargs="?", default=None, help="program to test"
    )
    parser.add_argument(
        "-s", "--spec", dest="spec", nargs="?", default="spec.txt", help="path to spec"
    )
    parser.add_argument(
        "-P",
        "--pattern",
        ...
        nargs="?",
        default=None,
        help="limit to sections matching regex pattern",
    )
    parser.add_argument(
        "--library-dir",
        dest="library_dir",
    ...
    parser.add_argument(
        "--debug-normalization",
        dest="debug_normalization",
        action="store_const",
        const=True,
        default=False,
        help="filter stdin through normalizer for testing",
    )
    parser.add_argument(
        "-n",
        "--number",
...
def print_test_header(test):
    out(
        f"Example {test['example']} "
        f"(lines {test['start_line']}-{test['end_line']}) {test['section']}\n"
    )
...
def do_test(test, normalize, prev_result):
    ...
    if retcode != 0:
        if prev_result != "error":
            ...
            out(f"program returned error code {retcode}\n")
    ...
    if actual_html != expected_html:
        if prev_result != "fail":
            ...
            for diffline in unified_diff(
                expected_html_lines, actual_html_lines, "expected HTML", "actual HTML"
            ):
                out(diffline)
            out("\n")
        return "fail"
...
def get_tests(specfile):
    ...
    with open(specfile, encoding="utf-8", newline="\n") as specf:
        for line in specf:
            line_number = line_number + 1
            stripped = line.strip()
            if stripped == "`" * 32 + " example":
                state = 1
            elif state == 2 and stripped == "`" * 32:
                state = 0
