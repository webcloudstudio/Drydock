...
Technology reference for Amazon SQS with Python (boto3, inside the `marina` library). This file does not
change between projects.

Prerequisites: `stack/python.md`, `stack/marina-library.md`

SQS replaces Marina's file-based JSONL queue. The design goal is unchanged: a producer (a phone inside
the firewall, a remote cron job, a failed script) submits work at any time, even when the local consumer
is off; the local Marina agent drains the queue when it next runs. Producers and consumers both go
through `marina.queue` — never raw boto3.
...
exactly-once is a hard requirement. Marina's jobs (transcribe this, run that) are independent — Standard
is correct. Choose FIFO only for a specific ordered workflow, accepting its throughput limits.

**Why**: Standard queues are cheaper, higher-throughput, and simpler. Marina's work items do not depend
...
```python
# Inside marina.queue.drain — shown for the rule only
...
```hcl
resource "aws_sqs_queue" "voice_dlq" { name = "marina-${var.project}-voice-dlq" tags = local.marina_tags }

resource "aws_sqs_queue" "voice" {
  name                       = "marina-${var.project}-voice"
...
**Rule**: `marina.queue.drain()` is safe to run twice — handlers key on the message `id` and skip
...
## 7. Access Through the Marina Library

**Rule**: Producers call `mar.queue.submit(...)`; the local consumer calls `mar.queue.drain(...)`. Queue
URLs, regions, and boto3 clients live inside `marina`. Project code never holds a queue URL.
...
|---|---|---|
| Queue | `marina-{project}-{queue}` | `marina-market-voice` |
| Dead letter queue | `marina-{project}-{queue}-dlq` | `marina-market-voice-dlq` |

Standard Marina tags on every queue and DLQ.
...
- [ ] All access through `marina.queue`; no queue URL in project code
